...
echo "<tr><th rowspan=2>序号</th><th rowspan=2>LED站点屏幕</th>";

$con = mysqli_connect('127.0.0.1','root','STdg123!','BAB');
	mysqli_query($con,'set names utf8');
	$sql = 'select `campaignID` from `LEDadCopyId` group by `campaignID`';
	...
	echo "<br><br>";
	echo "<table class=\"bordered\" cellpadding=\"0\" cellspacing=\"0\">";*/
        $sqls = "SELECT `screenID` FROM `LEDadCopyId` group by `screenID`";
	$ress = mysqli_query($con,$sqls);
		...
		$sqlCam = "select `campaignID` from `LEDadCopyId` group by `campaignID`";
		...
		{	
			...
			$CampaignID = $sql_arrCam['campaignID'];
    			$sqlc = "select `historyCount`,`planCount` from `LEDadCopyId` where `campaignID` = '{$CampaignID}' and `screenID` = '{$screen}'";
